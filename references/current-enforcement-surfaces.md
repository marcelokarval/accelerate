...
- use project mixins or model base classes instead of repetition
- after the second repetition, suggest mixin/heredity extraction explicitly
- review all meaningful `save()` calls for race or side-effect risk
...
- use the project shortcuts, base models, and canonical helpers
...
- use the project compounds and enhanced components before inventing new ones
- mandatory source ladder for missing components:
  1. official shadcn / existing canonical primitives
  2. acceptable third-party source when justified
  3. only then propose custom construction from base components
- avoid `div soup`
...
- stack-specific governance skills should be loaded when the branch risk demands
  them
- `Contract Correctness` when presenter, props, routes, identifiers, or action
  URLs are touched
