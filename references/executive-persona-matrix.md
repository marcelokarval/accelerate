...
| `Implementation Designer` | `planning-with-files`, `executing-plans` when accepted | `visual-modeling`, `ascii-wireframe`, contract/governance sidecars | execution design | `Implementation Design Packet` |
| `Visual Modeler` | `visual-modeling` | `ascii-wireframe` for UI/product surfaces; `architecture`, `database-design`, `subagent-governance`, and security/data skills as needed | ERD, sequence, state machine, swimlane, agent communication, trust boundary, or topology needed before implementation/review | `Visual Modeling Packet` |
| `Wireframe / Design Contract Extractor` | `ascii-wireframe` | `visual-modeling`, `figma-node-fidelity`, `front-react-shadcn`, `product-runtime-review` | structural UI uncertainty | design / wireframe packet |
