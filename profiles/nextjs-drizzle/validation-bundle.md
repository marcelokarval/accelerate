# Next.js Drizzle Validation Bundle

Use this bundle when a project declares Next.js + Drizzle as its active profile.
...
- Server Action return-value filtering proof when mutations return data to the
  client
- Drizzle schema/type proof when schema files change
- Drizzle migration strategy declaration when schema or migration behavior is in
  scope: `push`, `generate+migrate`, `pull`, `export`, external migration tool,
  or runtime migration
- Drizzle migration generation/check proof when schema changes
- Drizzle migration apply proof when migrations change runtime database state
- database connection proof for data-access changes
- SQL/index/constraint proof for relational invariants and performance-sensitive
  paths
...
- auth/session: signed-in, signed-out, unauthorized, and ownership-denied state
  proof when relevant
- database calls: Drizzle query proof, schema/migration alignment proof, and an
  explicit blocked reason when local credentials are unavailable
- data access modules: proof that privileged Drizzle/env/session access is
...
- `Drizzle Schema=<present|missing|not-applicable>`
- `Drizzle Migration Strategy=<present|missing|blocked|not-applicable>`
- `Drizzle Migration Generation=<present|missing|blocked|not-applicable>`
- `Drizzle Migration Apply=<present|missing|blocked|not-applicable>`
...
- `SQL/Index/Constraint Proof=<present|missing|blocked|not-applicable>`
