...
- [ ] Write checks for the three routes, direct zero-agent policy, scoped one-sidecar cap, and parallelism preconditions.
- [ ] Run the test and confirm it fails because the route contract is absent.
- [ ] Add the smallest routing language to the governing documents.
- [ ] Re-run the focal test and existing classification/delegation tests.
...
- [ ] Add route, budget, and one-line delegation basis to the existing Branch Entry Packet.
- [ ] Add the route decision to the quick map and portable runtime summary.
- [ ] Keep the Direct Fast Path packet compact and retain stricter local authority precedence.
- [ ] Re-run focal and relevant regression tests.
...
- [ ] Run the complete repository suite and formatting/link checks relevant to the changed docs.
- [ ] Obtain independent specification and quality review after the candidate is stable.
- [ ] Commit the bounded source change, merge it without disturbing pre-existing worktree changes, then synchronize and verify the global runtime mirror.
