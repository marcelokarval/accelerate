...
| workflow catalog | strong | native | near parity | focused retest exposed a real under-call around benchmark result registration; native catalog and quick map now include an explicit benchmark rerun / result-registration lane, and post-correction explicit/light reruns now select the right lane family, but behavioral promotion still needs repeated durable comparative proof |
