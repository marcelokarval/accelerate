...
| autoresearch / self-evolution | strong | native | near parity | focused durable-evidence rerun showed both local and legacy judges aligned on `near parity`; the local now has a narrow durable-capture rule, but still needs a follow-up rerun proving parity/promotion evidence no longer remains chat-only, `/tmp`-only, or scratch-only |
