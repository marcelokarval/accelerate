...
| autoresearch / self-evolution | strong | native | near parity | focused durable-evidence rerun and later natural follow-up after the parity-gate promotion both kept this surface at `near parity`; the local has the durable-capture rule, but still needs proof from a real recurring failure or workflow-learning event rather than adjacent-surface success |
