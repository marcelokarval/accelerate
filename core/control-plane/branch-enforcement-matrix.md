...
Every mutation row also inherits `Specification Entry Gate`, `SDD Mode Gate`,
`Decision Artifact Gate`, `Test Design Gate`, and `TDD Entry Gate` before
implementation. The Engineering Artifact Manifest must name `micro`,
`standard`, `hierarchical`, or `critical`, point to an `accepted` or
`implementing` design authority, and carry complete dispositions and
traceability. Direct-fast-path mutation uses a micro Spec Capsule; the route
does not waive semantic SDD or issue bootstrap.
...
## Visual Modeling Gate Rule

When structural truth is part of the branch, add `visual-modeling` and `Visual Modeling Gate` before implementation or closure.

This is required for:

- schema/database/table/constraint decisions (`ERD`);
- ORM/service/data-access boundary decisions (`ORM lifecycle`);
- provider callback, payment, auth, webhook, queue, or agent ordering decisions (`sequence diagram`);
- lifecycle/status/terminal-state decisions (`state machine`);
- user/lead/owner/operator/provider responsibility over time (`swimlane/journey`);
- multi-agent delegation or lane-handoff authority (`agent communication`);
- security-sensitive auth/upload/export/billing/PII paths (`trust boundary/dataflow`);
- infrastructure/provider/runtime shape (`deployment/runtime topology`);
- issue/proof/control-plane truth (`governance topology`).

Expected artifact: `Visual Modeling Packet` from `core/runtime-packets/visual-modeling-packet.md`, or an explicit no-diagram exception in the Branch Entry Packet.
...
| --- | --- | --- | --- | --- | --- |
| trivial bounded | none beyond the directly relevant stack skill | honest classification; `Local Workspace Entry Gate` when a governed target repo is mutated; `Issue Bootstrap Gate`, `Specification Entry Gate`, `SDD Mode Gate`, `Decision Artifact Gate`, `Test Design Gate`, and `TDD Entry Gate` when the slice mutates code, workflow seeds, or living docs; `Review Readiness Gate` and `Timeline Continuity Gate` when the slice stops being a one-shot bounded change; validation gate matrix when backend/frontend runtime is in scope; comparative review posture when the slice ceases to be obviously one-shot | bounded scope statement; governing issue when mutating; micro Spec Capsule and compact Engineering Artifact Manifest for direct-fast-path mutation; local workspace decision when target repo governance applies; readiness dashboard when local workspace status is active; `Requested-Vs-Implemented Packet` when the slice is review-bearing or multi-step; defect disposition when concrete defects were found | manifest validation plus direct code/test/runtime proof; required validation commands for the slice class; commit traceability when mutating; corrected-state reproof after any correction | hidden cross-surface drift; local workspace skipped in a governed target repo; trivial run attempted without governing issue or micro semantic SDD; mode `none`; readiness/timeline ignored after the slice became multi-step; validation stack under-run; stale proof after correction; slice promoted on recap language with no comparative or defect posture |
...
| prompt upgrade approval | `prompt-hardening`; active stack skills only for context, not execution | `Prompt Upgrade Approval Gate`; `Artifact Sufficiency Check` when source docs or repo context drive the upgraded prompt; `Response Locale Gate` | improved prompt; `Prompt Upgrade Approval Packet`; post-approval analysis report, executive plan, and task ledger when approved | explicit user approval before post-approval artifacts or execution; persisted artifacts after approval; on-screen plan/task presentation; separate commit/push authorization when requested | raw prompt executed before approval; approval assumed; stack basis omitted; artifacts skipped; implementation started before execution authorization; commit/push inferred from earlier approval |
...
| execution-to-spec loop | `executing-plans`; active stack skills for touched code; browser/QA adjacent skills when runtime proof is active | `Execution-To-Spec Loop Gate`; `One-Shot Side-By-Side Gate`; `Review Readiness Gate`; `Timeline Continuity Gate`; `Visual Regression Proof Gate` when UI changes; active correction / defect disposition when proof finds defects; `Manual Review Contradiction Gate` when later review contradicts closure | executive plan; task ledger; `Execution-To-Spec Loop Packet`; one-shot side-by-side review packets; defect ledger; correction loop packets; final forensic reconciliation | implementation proof + backend/frontend QA + browser proof when runtime is in scope + visual proof when UI changed + corrected-state reproof for each fixed defect | loop stopped after first proof; task ledger missing; browser/QA findings not fed into correction; corrections without reproof; final forensic reconciliation missing; planned outcome not reconciled; done claim contradicted by later review |
| manual review contradiction | active stack skills for contradicted tasks; `systematic-debugging` for valid defects; review/governance skills for closure drift | `Manual Review Contradiction Gate`; `Execution-To-Spec Loop Gate` when reopened tasks need correction; active correction / defect disposition | `Manual Review Contradiction Packet`; reopened task ledger entries; defect ledger updates; corrected-state proof for valid findings | finding-by-finding classification + task status changes + coverage reconciliation + fresh proof for valid reopened findings | manual review findings summarized but not classified; done status left unchanged despite valid evidence; browser proof used to dismiss code/data bug; reopened task has no reproof |
| systemic UI inconsistency audit | `product-runtime-review`; active frontend stack skill; `tailwind-design-system`; `react-best-practices`; browser/QA adjacent skills when runtime proof is active | `Systemic UI Inconsistency Audit Gate`; `Execution-To-Spec Loop Gate` when correction loop delivery is requested; `Browser Truth Gate`; `Visual Regression Proof Gate`; `Artifact Sufficiency Check`; active correction / defect disposition | route/surface inventory; modal/dialog/popover inventory; source search packet; viewport matrix; `Systemic UI Inconsistency Audit Packet`; defect ledger; correction loop packets | browser proof across declared intensity and viewports + source evidence for hard-coded/mock/data-truth findings + corrected-state proof for fixed defects | broad audit claimed from samples; modals/popups missed; hard-coded data untriaged; database truth unproven; desktop-only responsive claim; visual findings not fed into correction |
| document cohesion / Markdown size audit | `codebase-inspection` when repo-wide counts are needed; active governance/review skills for affected docs | `Document Cohesion Size Gate`; `Artifact Sufficiency Check`; `Truth Ownership Check` when authority boundaries are unclear; `Living Docs Obligation` when docs are changed | line-count inventory; `Document Cohesion Size Packet`; split/merge/pointer decision; updated README/index surfaces when owner files change | line-count evidence + owner classification + preservation of examples, failure labels, packets, and proof requirements | mechanical split by line count; large live doctrine changed without cohesion classification; duplicate authority left unresolved; indexes not updated; context lost during split |
| agent promotion / bounded agent candidate | root-owned agent-factory planning; active governance/review skills for the proposed family; stack skills only when candidate proof depends on them | `Truth Ownership Check`; `Artifact Sufficiency Check`; `Segregated Review Authority`; root-only fallback requirement from the agent promotion contract | `Agent Promotion Packet`; candidate input/return contract; skill envelope; runtime adapter binding decision; empirical replay plan or evidence | promotion ladder state evidence + root-only fallback + review isolation plan + empirical replay before promotion | gap treated as promotion; candidate lacks input/return contract; runtime adapter aspirational; no root-only fallback; agent allowed to claim closure; promotion without replay |
...
| `architecture / governance doubt` | `architecture`, `visual-modeling` when structure/ownership/topology must be drawn, `governance-audit`; add `api-surface-governance`, `dependency-governance`, or `validation-governance` when that stack truth is involved | `Truth Ownership Check`; `Stack Adherence`; `Visual Modeling Gate` when structure/ownership/topology is active | decision record, boundary matrix, ADR-oriented conclusion, Visual Modeling Packet when active | code/doc evidence and explicit judgment | conclusion still implicit, stack truth unresolved, or structural model skipped despite unresolved ownership/topology |
...
| runtime / product-heavy flow | `product-runtime-review`; `visual-modeling` when user/lead/operator journey, sequence, or lifecycle state is the decision surface; `anti-abuse-review` when sensitive; `security-patterns` when ownership, auth, secret, or race posture is part of the risk | `Product-Correctness`; `Visual Modeling Gate` when journey/sequence/lifecycle truth is active; `UX/UI Fullstack Surface Gate` when backend truth, frontend state, and UX/UI runtime behavior all affect closure; `Artifact Sufficiency Check` when the branch depends on artifacts; `Containment Policy Check` when hostile behavior is part of the diagnosis | runtime narrative, Visual Modeling Packet when active, possible wireframe for complex flow, UX/UI Fullstack Surface Packet when fullstack state behavior is active | browser truth before closure; backend truth and frontend state evidence when UX/UI fullstack is active | technically correct but poor product behavior; backend truth disconnected from UI state; journey/sequence/lifecycle modeled only in prose when structure is active |
| untrusted ingress / upload / import / media ingestion | `untrusted-ingress-hardening`; `visual-modeling` when dataflow/trust boundary is not already explicit; `anti-abuse-review` when user-facing; `security-patterns` when ownership, serving posture, or remote-fetch risk exists; `product-runtime-review` when user-facing | `Untrusted Ingress Gate`; `Visual Modeling Gate` when trust/dataflow is active; `Artifact Sufficiency Check` when parser/normalization/storage decisions are central | ingress packet covering allowed formats, limits, transformation posture, storage/serving posture, Visual Modeling Packet when trust/dataflow is active | code proof + parser/validation evidence + browser/runtime proof when user-facing | frontend-only validation, weak MIME trust, unbounded imports, metadata/privacy leakage, unsafe serving posture, or trust/dataflow boundaries hidden in prose |
...
| query / contract-sensitive backend | active backend/data stack skill such as `adonisjs-patterns`, `django-service-patterns`, `prisma-patterns`, or `drizzle-patterns`; `visual-modeling` when schema, ORM lifecycle, service/data access, or query topology is the decision surface; active integration skill such as `django-inertia-integration` when relevant; `server-prop-governance`; `sql-optimization-patterns` when relational fetch risk exists; `security-patterns` when ownership, secret, or race posture is part of the risk; `financial-source-truth` when money is involved | `Truth Ownership Check`; `Visual Modeling Gate` when schema/ORM/query topology is active; `Query Shape Proof`; `Django Backend Safety Gate` and `Django ORM Query Shape Gate` when Django ORM/runtime is in scope; `Ownership / IDOR Gate` when scoped objects are read or mutated; `Django Service Layer Contract` when Django business logic, mutations, tasks, or admin actions change; `Backend Subagent Readiness Contract` before creating or promoting Django/backend subagents; `Backend Product Contract Protocol` for sensitive self-service/recovery surfaces; `Secrets & Config Hygiene Check` when env/provider configuration is touched; `Concurrency Integrity Check` when critical mutable state is involved; backend validation gate matrix | contract packet, Visual Modeling Packet when active, query-shape proof, ownership matrix, service-layer proof when Django is active, backend subagent readiness packet when subagents are in scope | tests, code inspection, query evidence, `EXPLAIN` when needed, backend validation stack, negative ownership tests for scoped Django surfaces | hidden N+1, weak ownership, unsafe contract drift, unjustified concurrency posture, omitted backend validation, missing ERD/ORM model when structural truth is active, Django service-layer bypass, or backend subagent promoted before rails are enforceable |
