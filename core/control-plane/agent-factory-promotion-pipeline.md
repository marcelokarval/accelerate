...
| Runtime binding | Runtime adapter, invocation contract, process lifecycle, cleanup trap, provider write boundaries, and monitoring. | `core/control-plane/runtime-adapter-maturity-dashboard.md`; adapter manifests; runtime packets. | Binding is promoted only after actual bounded runtime proof; this cycle provides criteria only. | Demote if binding lacks cleanup, leaves processes idle, or claims unavailable runtime. | Kill unmanaged processes; record idle-agent closure; clean temp files and private outputs. |
...
| Agent factory role promotion | `scaffolded` | This file; `core/delegation/agent-factory-promotion-pipeline.md` pointer | No runtime binding or autonomous agent runtime proof exists in this cycle. | Run proof replay for one bounded role with local skill envelope, negative fixtures, cleanup proof, and root acceptance. | agent-factory governance |
