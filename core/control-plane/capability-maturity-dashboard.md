...
2. Keep Linear structured paths `planned` until non-sensitive live fixture proof is
   recorded with durable proof locators; RC18 repo-local live preflight records
   exact missing non-sensitive gates, but live availability remains unpromoted
   while `LINEAR_API_KEY` and safe fixture settings are absent.
