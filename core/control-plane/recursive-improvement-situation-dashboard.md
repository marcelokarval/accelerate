...
| Linear MCP writes | `planned` | `core/control-plane/capability-maturity-dashboard.md` records repo-local structured non-LLM GraphQL helper shape for read/create/artifact-comment/closure-comment/status-transition and keeps live availability unpromoted; `planning/evidence/dated-proof-appendix/linear-mcp-live-validation-2026-05-08.md` records live proof blocked by missing `LINEAR_API_KEY`. | No non-sensitive live Linear fixture proof has been recorded; helper shape exists but provider availability remains unproven. | Prove read/create/artifact-comment/closure-comment/status-transition on a non-sensitive Linear fixture before any `available` promotion. | workflow adapter implementation subagent + provider-boundary/governance reviewer |
...
| Skill sync topology | `linked` | `core/control-plane/skill-sync-topology.md` defines repo-local source of truth, repo-outward generated export direction, forbidden user-home authority, drift detection, sync boundaries, and promotion criteria; RC9 proof is `planning/evidence/dated-proof-appendix/skill-export-proof-2026-05-08.md` and `bash tests/skill-export-proof.sh`. | Generated skill export is available only as a repo-local proof artifact; host/user-home catalogs remain non-authoritative and unpromoted. | Run the provenance/drift proof against an explicitly approved generated host target before any host runtime export promotion. | skill governance subagent + reviewer sidecar |
...
- Linear MCP writes remain `planned` for repo-local read/create/artifact-comment/closure-comment/status-transition helper shape until live non-sensitive fixture proof exists.
...
1. Linear live fixture proof: prove repo-local read/create/artifact-comment/closure-comment/status-transition helpers against a non-sensitive Linear fixture once `LINEAR_API_KEY` and safe target status are available.
2. Browser proof: keep server readiness/capture/failure/cleanup monitoring covered and avoid promoting persistent E2E beyond fixture proof.
3. Skill sync host export: run the RC9 provenance/drift proof against an explicitly approved generated host target before any host runtime availability claim.
4. Agent factory runtime binding: keep bounded proof-auditor at fixture-scoped `proof-replay` until runtime lifecycle, cleanup/idle-agent handling, demotion, and root acceptance are proven.
5. Semantic negative fixtures for packets and gates.
6. Runtime adapter maturity dashboard follow-through: prove or demote individual runtime adapters using the linked dashboard.
