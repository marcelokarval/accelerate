...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Browser proof packet and browser truth contract | `linked` | `core/runtime-packets/browser-proof-packet.md`; `adapters/runtime/browser/browser-truth-contract.md`; `tests/browser-proof-monitoring.sh`; RC8 evidence in current cycle plan/ledger | Server readiness and browser-proof monitoring improvements are explicit; persistent browser/E2E availability remains unpromoted unless fixture capture proof exists. | Promote a browser capture path only when bounded server readiness, console/network output, failure packet capture, cleanup, and corrective routing are contract-tested. | Demote to `substitute` if proof only contains screenshots, stale artifacts, no cleanup result, or no server readiness result. | Remove stale captures from generated proof locations; retain only durable packet contracts and non-sensitive fixtures. | runtime/browser governance + QA proof lane |
...
| Autonomous agent runtime | `blocked` | `core/control-plane/agent-factory-promotion-pipeline.md`; RC10 fixture replay appendix `planning/evidence/dated-proof-appendix/agent-factory-replay-2026-05-08.md` proves only candidate proof-replay criteria. | This repository is still pre-agents; RC10 replay is fixture-scoped and no autonomous runtime binding exists. | Requires candidate role intake, skill envelope, proof replay, runtime binding, cleanup/idle-agent handling, demotion criteria, monitoring, and root acceptance. | Any claim of autonomous execution without runtime binding proof demotes to `blocked` and triggers forensic review. | Kill unmanaged processes; retire unbound role drafts; preserve only approved docs/tests. | agent-factory governance |
