---
name: mcporter
description: Codex-native guidance for using the mcporter CLI to list, configure, auth, and call MCP servers/tools directly from the terminal, including ad-hoc servers and config edits.
version: 1.0.0
author: community
license: MIT
metadata:
  ...
  related_skills: [native-mcp, github-auth]
...
mcporter auto-discovers servers configured by other MCP clients on the machine.
To find new servers to use, browse MCP registries, then connect ad-hoc:
...
# Machine-readable output (recommended for Codex)
...
- `native-mcp` for durable Codex-native MCP configuration in `~/.codex/config.toml`
- `github-auth` and `github-*` skills when the ad-hoc MCP experiment turns into real GitHub task work
